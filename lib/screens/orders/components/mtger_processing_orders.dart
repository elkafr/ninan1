...
class MtgerProcessingOrders extends StatefulWidget {
  @override
  _MtgerProcessingOrdersState createState() => _MtgerProcessingOrdersState();
}

class _MtgerProcessingOrdersState extends State<MtgerProcessingOrders> {
 ...
 ProgressIndicatorState _progressIndicatorState;
  ...
  Future<List<Order>> _getOrderList() async {
    ...
    Map<String, dynamic> results = _appState.filterOrders==null || _appState.filterOrders==10?await _services.get(
        'https://ninanapp.com/app/api/mtger_dis_buy?lang=${_appState.currentLang}&mtger_id=${_storeState.currentStore.mtgerId}&page=1&done=1'):await _services.get(
                                              ...
                                              child: Row(
                                                children: <Widget>[
                                                  ...
                                                  Container(
                                                    ...
                                                    width: 150,
                                                    height: 45,
                                                    child: snapshot.data[index].carttStateNumber=="0"?CustomButton(
                                                      btnLbl: "بدء تجهيز الطلب",
                                                      onPressedFunction: () async {


                                                        _progressIndicatorState.setIsLoading(true);
                                                        var results = await _services.get(
                                                            'https://ninanapp.com/app/api/mtger_updateRequestState?mtger_id=${_storeState.currentStore.mtgerId}&cartt_fatora=${snapshot.data[index].carttFatora}&cartt_seller=${snapshot.data[index].carttSeller}&cartt_done=1&lang=${_appState.currentLang}');
                                                    ...
                                                    ):snapshot.data[index].carttStateNumber=="1"?CustomButton(
                                                      btnLbl: "تم انتهاء التجهيز",
                                                      onPressedFunction: () async {


                                                        _progressIndicatorState.setIsLoading(true);
                                                        var results = await _services.get(
                                                            'https://ninanapp.com/app/api/mtger_updateRequestState?mtger_id=${_storeState.currentStore.mtgerId}&cartt_fatora=${snapshot.data[index].carttFatora}&cartt_seller=${snapshot.data[index].carttSeller}&cartt_done=6&lang=${_appState.currentLang}');
                                                        _progressIndicatorState.setIsLoading(false);
                                                        if (results['response'] == '1') {
                                                      ...
                                                      child: Text("بانتظار الكابتن للاستلام",style: TextStyle(color: cPrimaryColor),),
                                                    ),
                                                  ),
                                                ],
                                              ),
