...
class FilterOpen extends StatefulWidget {
  const FilterOpen({Key key}) : super(key: key);
  @override
  _FilterOpenState createState() => _FilterOpenState();
}

class _FilterOpenState extends State<FilterOpen> {
            ...
            children: <Widget>[
              ...
              Container(
                ...
                child: Column(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: <Widget>[
                    Text("حالة المتجر",style: TextStyle(color: cText,fontSize: 16,fontWeight: FontWeight.bold),),
                    Padding(padding: EdgeInsets.all(5)),
                    Text("حدد حالة المتجر الحالية",style: TextStyle(color: cHintColor,fontSize: 13),),
                  ],
                ),
              ),
                  ...
                  ListTile(
                    title: GestureDetector(
                      child: Container(
                        ...
                        child: Text("مفتوح",style: TextStyle(color: cPrimaryColor,fontSize: 15),),
                      ),
                      onTap: () async{

                        _progressIndicatorState.setIsLoading(true);
                        var results = await _services.get(
                            'https://ninanapp.com/app/api/mtger_changeState?mtger_id=${_storeState.currentStore.mtgerId}&mtger_state=open&lang=${_appState.currentLang}');
                        _progressIndicatorState.setIsLoading(false);
                        if (results['response'] == '1') {

                          _storeState.updateMtgerState("open");
                          SharedPreferencesHelper.save(
                              "store", _storeState.currentStore);
                  ...
                  ListTile(
                    title: GestureDetector(
                      child: Container(
                        ...
                        child: Text("مشغول",style: TextStyle(color: Colors.grey,fontSize: 15),),
                      ),
                      onTap: () async{

                        _progressIndicatorState.setIsLoading(true);
                        var results = await _services.get(
                            'https://ninanapp.com/app/api/mtger_changeState?mtger_id=${_storeState.currentStore.mtgerId}&mtger_state=busy&lang=${_appState.currentLang}');
                        _progressIndicatorState.setIsLoading(false);
                        if (results['response'] == '1') {

                          _storeState.updateMtgerState("busy");
                          SharedPreferencesHelper.save(
                              "store", _storeState.currentStore);
                          showToast( results['message'], context);
                          Navigator.pop(context);
                  ...
                  ListTile(
                    title: GestureDetector(
                      child: Container(
                        ...
                        child: Text("مغلق",style: TextStyle(color: Colors.red,fontSize: 15),),
                      ),
                      onTap: () async{

                        _progressIndicatorState.setIsLoading(true);
                        var results = await _services.get(
                            'https://ninanapp.com/app/api/mtger_changeState?mtger_id=${_storeState.currentStore.mtgerId}&mtger_state=close&lang=${_appState.currentLang}');
                        _progressIndicatorState.setIsLoading(false);
                        if (results['response'] == '1') {

                          _storeState.updateMtgerState("close");
                          SharedPreferencesHelper.save(
                              "store", _storeState.currentStore);
                          showToast( results['message'], context);
                          Navigator.pop(context);
