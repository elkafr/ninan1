...
class MtgerRegisterCodeActivationScreen extends StatefulWidget {
  @override
  _MtgerRegisterCodeActivationScreenState createState() =>
      _MtgerRegisterCodeActivationScreenState();
}

class _MtgerRegisterCodeActivationScreenState
    extends State<MtgerRegisterCodeActivationScreen> with TickerProviderStateMixin {
               ...
               onTap: () async{
                 ...
                 _firebaseMessaging.getToken().then((token) async {
                   ...
                   var results = await _services.get(
                     '${Utils.MTGER_SENDCODE_URL}?mtger_id=${_appState.phoneSend}&token=${_appState.tokenSend}&lang=${_appState.currentLang}&key=$cKey',
                   );
                   _progressIndicatorState.setIsLoading(false);
                   if (results['response'] == '1') {
                     showToast(results['message'], context);
                     _storeState.setCurrentStore(Store(mtgerId:results['mtger_id'].toString() ));
        ...
        Container(
            ...
            child: CustomButton(
                ...
                onPressedFunction: () async {
                  _progressIndicatorState.setIsLoading(true);
                  var results = await _services.get(
                      'https://ninanapp.com/app/api/mtger_active_code?mtger_id=${_storeState.currentStore.mtgerId}&mtger_code=$_activationCode&lang=${_appState.currentLang}');
                  _progressIndicatorState.setIsLoading(false);
                  if (results['response'] == '1') {

                    _storeState.setCurrentStore(
                        Store.fromJson(results['mtger_details'])
                    );
